...
var g_wifi_connectionStatus_S1 = null;
var g_wifi_connectionStatus_S2 = null;
/*
    ...
    || (g_module.cradle_enabled && G_cradleStationStatus.cradlestatus == CRADLE_NETLINE_EXIST && G_cradleStationStatus.connectionmode != CRADLELANONLY && (CRADLEAUTOMODE != G_cradleStationStatus.connectionmode)))) {
        ...
        */
        ...
        if ( ( $.browser.msie && ($.browser.version == '6.0') )||  g_allConnStatus != signal_strength) {
            ...
            } else if(webui_mode == 'new' || webui_mode == 'material') {
                if(signal_strength == 0)
                    $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M1 16h15V1z"/></svg>');
                else if(signal_strength == 1)
                    $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M1 16h15V1z"/><path d="M1 16h7V9z"/></svg>');
                else if(signal_strength == 2)
                    $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M1 16h15V1z"/><path d="M1 16h9V7z"/></svg>');
                else if(signal_strength == 3)
                    $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M1 16h15V1z"/><path d="M1 16h11V5z"/></svg>');
                else if(signal_strength == 4 || signal_strength == 5)
                    $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path d="M1 16h15V1z"/></svg>');
            }
            ...
            if (ret.type == "response") {
                ...
                if ( ( $.browser.msie && ($.browser.version == '6.0')) ||  g_allConnStatus != wifi_Signal) {
                    ...
                    if(webui_mode == 'old') {
                        ...
                        if(wifi_Signal == 0)
                            $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/></svg>');
                        else if(wifi_Signal == 1)
                            $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/><path d="M5.24 11.31L9 16l3.76-4.68C12.57 11.17 11.14 10 9 10c-2.14 0-3.57 1.17-3.76 1.31z"/></svg>');
                        else if(wifi_Signal == 2)
                            $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/><path d="M3.98 9.75l5.01 6.24L9 16v-.01l5.01-6.24C13.76 9.56 11.85 8 9 8 6.15 8 4.24 9.56 3.98 9.75z"/></svg>');
                        else if(wifi_Signal == 3)
                            $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/><path d="M2.73 8.19L9 15.99V16l.01-.01 6.27-7.8C14.96 7.94 12.56 6 9 6S3.04 7.94 2.73 8.19z"/></svg>');
                        else if(wifi_Signal == 4)
                            $('#status_img').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#757575" width="118" height="118" viewBox="0 0 18 18"><path d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/></svg>');
                    }
                }
            }
        }, {
...
function index_checkConnectionStatusChange() {
    ...
    if (g_connectionStatus_S2 == null || g_wifi_connectionStatus_S2 == null) {
        g_connectionStatus_S1 = currentStatus.ConnectionStatus;
        g_wifi_connectionStatus_S1 = currentStatus.WifiConnectionStatus;
        if (currentStatus.ConnectionStatus == MACRO_CONNECTION_CONNECTED || currentStatus.WifiConnectionStatus == MACRO_CONNECTION_CONNECTED) {
            index_gotoOperatorHomePage(false);
            main_getAdvertisement();
        }
    } else {
        g_connectionStatus_S1 = g_connectionStatus_S2;
        g_wifi_connectionStatus_S1 = g_wifi_connectionStatus_S2;
    }
    g_connectionStatus_S2 = currentStatus.ConnectionStatus;
    g_wifi_connectionStatus_S2 = currentStatus.WifiConnectionStatus;
    if ((g_connectionStatus_S1 != MACRO_CONNECTION_CONNECTED &&
    g_connectionStatus_S2 == MACRO_CONNECTION_CONNECTED)||
    (g_wifi_connectionStatus_S1 !=  MACRO_CONNECTION_CONNECTED && g_wifi_connectionStatus_S2 == MACRO_CONNECTION_CONNECTED)) {
...
function GetPLMN() {
    ...
    (g_module.cradle_enabled && G_cradleStationStatus.cradlestatus == CRADLE_NETLINE_EXIST && G_cradleStationStatus.connectionmode != CRADLELANONLY && (CRADLEAUTOMODE != G_cradleStationStatus.connectionmode))))
            ...
            if ('response' == plmn_ret.type) {
                ...
                null == plmn_ret.response.State)
                    ...
                    plmn_ret.response.FullName.length > 0)
                        ...
                        plmn_name = '';
                    $.each(operatorlist.find('Item'), function(n, subValue) {
                        if(
                        (($(subValue).attr('shortdescr') && plmn_ret.response.ShortName &&
                        $(subValue).attr('shortdescr').toLowerCase() == plmn_ret.response.ShortName.toLowerCase()) ||
                        ($(subValue).attr('fulldescr') && plmn_ret.response.FullName &&
                        $(subValue).attr('fulldescr').toLowerCase() == plmn_ret.response.FullName.toLowerCase()) ||
                        ($(subValue).attr('fulldescr') && plmn_ret.response.ShortName && 
                        $(subValue).attr('fulldescr').toLowerCase() == plmn_ret.response.ShortName.toLowerCase()) ||
                        ($(subValue).attr('shortdescr') && plmn_ret.response.FullName &&
                        $(subValue).attr('shortdescr').toLowerCase() == plmn_ret.response.FullName.toLowerCase())) &&
                        typeof($(subValue).attr('picture')) != 'undefined' &&
                        $(subValue).attr('picture').length > 0
                        ) {
                            plmn_name = $(subValue).attr('picture');
                            plmn_img = true;
                        }
                    });
                }
        ...
        if(plmn_img)
            $('#index_plmn_name').html("<img onload = 'fixPNG(this)' src = '" + plmn_name + "' style='max-width: 100%;'>");
        else
    ...
    getAjaxData('api/device/information', function($xml) {
        ...
        if (ret.type == 'response') {
            if (typeof(ret.response.Msisdn) != 'undefined' && ret.response.Msisdn != '') {
                $('#my_number').show();
                $('#MyNumber').text(ret.response.Msisdn);
            } else {
                $('#MyNumber').empty();
                $('#my_number').hide();
            }
...
function getLoginUitl(login_id, loginTrafficxml) {
    if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
        if (loginTrafficxml >= g_monitoring_dumeter_tb) {
            $('#' + login_id).text(common_unit_tb + ' ' + formatFloat(parseFloat(loginTrafficxml) / g_monitoring_dumeter_tb, 2));
        } else if (loginTrafficxml < g_monitoring_dumeter_tb && loginTrafficxml >= g_monitoring_dumeter_gb) {
            $('#' + login_id).text(common_unit_gb + ' ' + formatFloat(parseFloat(loginTrafficxml) / g_monitoring_dumeter_gb, 2));
        } else if (loginTrafficxml < g_monitoring_dumeter_gb && loginTrafficxml >= g_monitoring_dumeter_mb) {
            $('#' + login_id).text(common_unit_mb + ' ' + formatFloat(parseFloat(loginTrafficxml) / g_monitoring_dumeter_mb, 2));
        } else if (loginTrafficxml < g_monitoring_dumeter_mb && loginTrafficxml >= g_monitoring_dumeter_kb) {
            $('#' + login_id).text(common_unit_kb + ' ' + formatFloat(parseFloat(loginTrafficxml) / g_monitoring_dumeter_kb, 2));
        } else if (loginTrafficxml < g_monitoring_dumeter_kb) {
            $('#' + login_id).text(common_unit_byte + ' ' + formatFloat(parseFloat(loginTrafficxml), 2));
        }
    } else {
        if (loginTrafficxml >= g_monitoring_dumeter_tb) {
...
function index_sendNetWorkAction() {
    ...
    if(connectMode == '1' && networkAction.dataswitch == MACRO_NETWORK_ON) {
        checkRoamWarn( function() {
            var dialup_xml = object2xml('request', g_dialup_action);
            saveAjaxData('api/dialup/dial', dialup_xml, function($xml) {
                var dialup_ret = xml2object($xml);
                g_isPostAjax = false;
                if (isAjaxReturnOK(dialup_ret)) {
                    if(MACRO_NETWORK_OFF == g_dialup_action.Action) {
                        $('#mobile_connect_btn').removeClass('mobile_connect_btn_on');
                    } else {
                        $('#mobile_connect_btn').addClass('mobile_connect_btn_on');
                    }
    ...
    if (!g_module.dataswitch_enabled) {
        ...
        $('#mobile_connect_btn').live('click', function() {
            ...
            if($(this).hasClass("mobile_connect_btn_on")) {
                g_dialup_action.Action = MACRO_NETWORK_OFF;
                g_network_action.dataswitch = MACRO_NETWORK_OFF;
            } else {
                g_dialup_action.Action = MACRO_NETWORK_ON;
...
$(document).ready( function() {
	...
	if(typeof(user_config.main_bg) != 'undefined') {
		main_bg = user_config.main_bg == 'true' ? true : false;
	}
	if(typeof(modsettings.main_bg_user) != 'undefined') {
		main_bg_user = modsettings.main_bg_user == 'true' ? true : false;
	}

    if(main_bg) {
        if(main_bg_user)
            $('.center_box').addClass('center_box_user_picture');
        else
            $('.center_box').addClass('center_box_standard_picture');
    } else if(webui_mode == 'new')
        $('.center_box').addClass('center_box_new');
    ...
    if (isNewVersionFound) {
        ...
        getAjaxData('api/monitoring/status', function($xml) {
            ...
            if ('response' == status.type) {
                ...
                g_wifi_connectionStatus_S2 = status.response.WifiConnectionStatus;
                if (status.response.ConnectionStatus == MACRO_CONNECTION_CONNECTED || status.response.WifiConnectionStatus == MACRO_CONNECTION_CONNECTED) {
                    index_gotoOperatorHomePage(false);
                }
            }
        }, {
        ...
        if (null == g_wifi_connectionStatus_S2) {
            g_wifi_connectionStatus_S2 = MACRO_CONNECTION_DISCONNECTED;
        }
    }
}
...
function formatComponentSize(bytes_number) {
    if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
        if (bytes_number >= g_TB) {
            return common_unit_tb + ' ' + formatFloat((parseFloat(bytes_number / (g_TB))) , 2);
        }
        if (bytes_number >= g_GB && bytes_number < g_TB) {
            return common_unit_gb + ' ' + formatFloat((parseFloat(bytes_number / (g_GB))) , 2);
        } else if (bytes_number >= g_MB && bytes_number < g_GB) {
            return common_unit_mb + ' ' + formatFloat((parseFloat(bytes_number / (g_MB))) , 2);
        } else if (bytes_number >= g_KB && bytes_number < g_MB) {
            return common_unit_kb + ' ' + formatFloat((parseFloat(bytes_number / (g_KB))) , 2);
        } else if (bytes_number < g_KB) {
            return common_unit_byte + ' ' + formatFloat((parseFloat(bytes_number / (1))) , 2);
        }
    } else {
        if (bytes_number >= g_TB) {
            return formatFloat((parseFloat(bytes_number / (g_TB))) , 2) + ' ' + common_unit_tb;
        }
        if (bytes_number >= g_GB && bytes_number < g_TB) {
            return formatFloat((parseFloat(bytes_number / (g_GB))) , 2) + ' ' + common_unit_gb;
        } else if (bytes_number >= g_MB && bytes_number < g_GB) {
            return formatFloat((parseFloat(bytes_number / (g_MB))) , 2) + ' ' + common_unit_mb;
        } else if (bytes_number >= g_KB && bytes_number < g_MB) {
            return formatFloat((parseFloat(bytes_number / (g_KB))) , 2) + ' ' + common_unit_kb;
        } else if (bytes_number < g_KB) {
            return formatFloat((parseFloat(bytes_number / (1))) , 2) + ' ' + common_unit_byte;
        }
    }
}
...
function showNewVersionDialog(destnation) {
    ...
    if(g_auto_update_enable == '1'){
        dialogHtml += "        <div style='padding:10px 20px;" + (webui_mode == 'old' ? 'width:374px;' : 'box-sizing:border-box;-moz-box-sizing:border-box;') + "' class='dialog_table noBorder' id='onlineupdate_prompt'>" + IDS_common_updateDialog_content + "</div>";
        dialogHtml += "        <div style='padding:10px 20px;" + (webui_mode == 'old' ? 'width:374px;' : 'box-sizing:border-box;-moz-box-sizing:border-box;') + "' class='dialog_table noBorder' id='onlineupdate_radio'>" ;
        ...
        dialogHtml += "        <div style='padding:10px 20px;" + (webui_mode == 'old' ? 'width:374px;' : 'box-sizing:border-box;-moz-box-sizing:border-box;') + "' class='dialog_table noBorder' id='onlineupdate_prompt'>" + IDS_common_new_version_auto_update + "</div>";
    }
    dialogHtml += "        <div class='dialog_table_bottom noBorder' style='" + (webui_mode == 'old' ? 'width:394px;' : 'padding-top:10px;padding-right:0;') + "'>";
...
function creatHomeContent() {
    ...
    if(search != 'yandex') {
        ...
        $('.search_t').removeClass('search_t').addClass('search_t_noyandex');
        $('img[src="../res/Button2.png"]').remove();
        $('.search_btn_text').removeClass('search_btn_text').addClass('search_btn_text_noyandex');
    }
    ...
    $('#new_content_right_top, #new_content_right_bottom').remove();
    ...
    if(webui_mode == 'old' && force_new_home) {
        ...
        if(webui_mode == 'new' && force_old_home) {
            $('.center_box script').remove();
            $('#new_content_right form').after($('.center_box').detach());
...
$(document).ready(function () {
    ...
    if(webui_mode == 'material') {
        $('.power_off_btn, .mobile_connect_btn, .wlan_connect_btn').each(function() {
            $(this).addClass('mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent').text($(this).attr('title'));
            if(typeof(componentHandler) != 'undefined')
                componentHandler.upgradeElement(this);
        });
        $('.login_info').addClass('mdl-grid').removeAttr('style');
        $('.login_info > div:visible').addClass('mdl-cell--6-col-desktop mdl-cell--4-col-tablet mdl-cell--2-col-phone').removeAttr('style');
        $('.login_info h2').each(function() {
            $(this).children('script').remove();
            $(this).replaceWith('<h5 id="' + $(this).attr('id') + '" class="mdl-color-text--accent">' + $(this).html() + '</h5>');
        });
        $('.login_info .label_left, .login_info .label_right').each(function() {
            $(this).children('script').remove();
	...
	$('#search_button').bind('click', function() {
		var a = $('.search_t, .search_t_noyandex').val();
                var final_link
                if(search == 'yandex')
    ...
    if(webui_mode == 'new' || webui_mode != 'material' && force_new_home) {
...
var search = 'yandex';
var main_bg = false;
var main_bg_user = false;
var force_new_home = false;
var force_old_home = false;

function getOperatorList() {
	var OperatorList = {};
	getConfigData('config/network/operatorlist.xml', function($xml) {
		if (typeof($xml.find('OperatorList')[0]) != 'undefined')
			OperatorList = $xml.find('OperatorList');
	}, {
		sync: true
	});
	return OperatorList;
