...
function initPage() {
    getAjaxData('api/security/sip', function($xml) {
        ...
        $('#enable_sip_alg').attr('checked', (g_sipData.SipStatus == 1) ? true : false);
        $('#sip_port').val(g_sipData.SipPort);
        checked_ck();
    });
}
...
function validateSIP() {
    ...
    if (sipEnable == 1) {
        if (isNaN(sipPort) || $.trim($('#sip_port').val()) == ''||$.trim($('#sip_port').val()).indexOf('.')>-1) {
            showErrorUnderTextbox('sip_port_prompt', firewall_hint_port_empty);
            $('#sip_port').focus();
            ret = false;
        }
        else if ((sipPort < 1) || (sipPort > 65535)) {
            showErrorUnderTextbox('sip_port_prompt', firewall_hint_port_number_valid_char);
            $('#sip_port').focus();
            ret = false;
        }
    }
