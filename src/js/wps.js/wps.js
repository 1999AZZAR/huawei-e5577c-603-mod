...
function wps_singleSSID_getConfig() {
    getAjaxData('api/wlan/basic-settings', function($xml) {
        ...
        getAjaxData('api/wlan/security-settings', function($xml) {
            var ret = xml2object($xml);
            g_wps_securityData_old = ret.response;
                    ...
                    if (returnData.type == 'response') {
                        ...
                        if (returnData.response.hash == hashData) {
                            ...
                            if (rets.type == 'response') {
                                ...
                                g_wps_securityData_old.WifiWpapsk = temp.WifiWpapsk;
                                g_wps_securityData_old.WifiWepKey1 = temp.WifiWepKey1;
                                g_wps_securityData_old.WifiWepKey2 = temp.WifiWepKey2;
                                g_wps_securityData_old.WifiWepKey3 = temp.WifiWepKey3;
                                g_wps_securityData_old.WifiWepKey4 = temp.WifiWepKey4;
                            }
                        }
                    }
                }, {
                        ...
                        if (returnData.response.hash == hashData) {
                            ...
                            if (rets.type == 'response') {
                                ...
                                $(g_wps_basicData).each( function(i) {
                                    g_wps_basicData[i].WifiWpapsk = secretKeyArray[i].WifiWpapsk;
                                    g_wps_basicData[i].MixWifiWpapsk = secretKeyArray[i].MixWifiWpapsk;
                                    g_wps_basicData[i].WifiWepKey1 = secretKeyArray[i].WifiWepKey1;
                                    g_wps_basicData[i].WifiWepKey2 = secretKeyArray[i].WifiWepKey2;
                                    g_wps_basicData[i].WifiWepKey3 = secretKeyArray[i].WifiWepKey3;
                                    g_wps_basicData[i].WifiWepKey4 = secretKeyArray[i].WifiWepKey4;
                                });
                            }
                        }
                    }
...
function saveWpsEnable(wps_enable) {
    if ($.isArray(g_wps_basicData)) {
        $(g_wps_basicData).each( function(i) {
            ...
            delete g_wps_basicData[i].WifiWpapsk;
            delete g_wps_basicData[i].MixWifiWpapsk;
            delete g_wps_basicData[i].WifiWepKey1;
            delete g_wps_basicData[i].WifiWepKey2;
            delete g_wps_basicData[i].WifiWepKey3;
            delete g_wps_basicData[i].WifiWepKey4;
            delete g_wps_basicData[i].WifiWep128Key1;
            delete g_wps_basicData[i].WifiWep128Key2;
            delete g_wps_basicData[i].WifiWep128Key3;
            delete g_wps_basicData[i].WifiWep128Key4;
        });
    } else {
        g_wps_basicData.WifiWpsenbl = wps_enable;
        delete g_wps_basicData.WifiWpapsk;
        delete g_wps_basicData.MixWifiWpapsk;
        delete g_wps_basicData.WifiWepKey1;
        delete g_wps_basicData.WifiWepKey2;
        delete g_wps_basicData.WifiWepKey3;
        delete g_wps_basicData.WifiWepKey4;
        delete g_wps_basicData.WifiWep128Key1;
        delete g_wps_basicData.WifiWep128Key2;
        delete g_wps_basicData.WifiWep128Key3;
        delete g_wps_basicData.WifiWep128Key4;
    }
    var postData = {
        Ssids: {
    ...
    saveAjaxData('api/wlan/multi-basic-settings', xmlStr, function($xml) {
        ...
        if (isAjaxReturnOK(ret) && wps_enable == 0) {
            ...
            getAjaxData('api/wlan/multi-basic-settings', function($xml) {
                ...
                if(g_wifiFeatureSwitch.wps_switch_enable == 1 && g_wps_basicData[0].WifiWpsenbl == '0') {
                    $('#appindisable').attr('checked', false);
                    $('input:not(#appinenable,#appindisable)').attr('disabled', true);
                    $('select:not(#lang)').attr('disabled', true);
                    button_enable('default_button', '0');
                    button_enable('rand_button', '0');
