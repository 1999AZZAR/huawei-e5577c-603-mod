...
function loadLangFile(lang) {
    var langFile = '../language/lang_' + (lang ? lang : LANGUAGE_DATA.current_language) + '.js';
    ...
    getAjaxData('api/monitoring/converged-status', function($xml) {
        ...
        if (ret.type == 'response') {
            ...
            if (LANGUAGE_DATA.current_language == 'ru_ru')
                loadLangFile('ru_ru.mod');
            else
                loadLangFile('en_us.mod');
        }
    }, {
        sync: true
    });
...
function main_executeBeforeDocumentReady() {
    ...
    factoryResetRequired = isFactoryResetRequired();
	...
	if(typeof(user_config.automaterial) != 'undefined' && user_config.automaterial == 'true' && $(window).width() < 601) {
        webui_mode = 'material';
    }
    if(current_href == 'antennapointing') {
        webui_mode = 'old';
    }
    ...
    if(webui_mode == 'old') {
        $('link[type="text/css"][rel="stylesheet"][href="../css/main.css"]').attr('href', '../css/main.old.css');
    } else if(webui_mode == 'material') {
        $('meta[http-equiv="Content-Type"][content="text/html; charset=utf-8"]').after('<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"/>');
        $('meta[name="viewport"][content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"]').after(
            '<meta name="theme-color" content="#03a9f4"/>' +
            '<meta name="msapplication-navbutton-color" content="#03a9f4">' +
            '<meta name="apple-mobile-web-app-capable" content="yes">' +
            '<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">'
        );
        $('link[type="text/css"][rel="stylesheet"][href="../css/main.css"]').before(
            '<link href="../fonts/iconfont/material-icons.css" rel="stylesheet">' +
            '<link href="../lib/dialog-polyfill.css" rel="stylesheet">'
        );
        $('link[type="text/css"][rel="stylesheet"][href="../css/main.css"]').attr('href', '../lib/material.min.css');
        $('link[type="text/css"][rel="stylesheet"][href="../lib/material.min.css"]').after(
            '<link href="../lib/material-file-input.css" rel="stylesheet">' +
            ...
            '<style>' +
                '#status_img_rat {' +
                    'font-size:20px;' +
                '}' +
                '.ieshow, .file-field .error_message, .stk_dialog, #stk_div_wrapper {' +
        ...
        dialogJS.src = "../lib/dialog-polyfill.js";
        document.head.appendChild(dialogJS);
        var materializeJS = document.createElement('script');
        materializeJS.src = "../lib/material.min.js";
        document.head.appendChild(materializeJS);
        var materializeFileInput = document.createElement('script');
        materializeFileInput.src = "../lib/material-file-input.js";
        document.head.appendChild(materializeFileInput);
        var materializeSelect = document.createElement('script');
        materializeSelect.src = "../lib/materialize.select.js";
        document.head.appendChild(materializeSelect);
        $('head').after('<div id="preloader"><div id="status"><div class="md-preloader"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="75" width="75" viewbox="0 0 75 75"><circle cx="37.5" cy="37.5" r="33.5" stroke-width="8"/></svg></div></div></div>');
    }

    var imgfixJS = document.createElement('script');
    imgfixJS.src = "../lib/imgfix.js";
    document.head.appendChild(imgfixJS);
...
function checkModuleEnable(current_page) {
    ...
    switch (current_page) {
        ...
        case "antennapointing":
            checkGotoHome(checkObjectExist(g_PageUrlTree, "antennapointing"));
            break;
        case "statistic":
        ...
        case "update_local":
            ...
            checkGotoHome(g_module.local_update_enabled && checkObjectExist(g_PageUrlTree, "settings", "setting_update"));
            break;
        case "dlna":
        ...
        case "ota":
            ...
            break;
        case "mods":
            checkGotoHome(g_module.modsettings_enabled && checkObjectExist(g_PageUrlTree, "mods"));
            break;
        case "sdcardsharing":
        ...
        case "quicksetup":
            ...
            checkGotoHome(checkObjectExist(g_PageUrlTree, "settings", "quick_setup") && g_Driver_classify != 'hilink');
            break;
        case "sntp":
        ...
        case "profilesmgr":
            ...
            break;
        case "autoimei":
            //g_PageUrlTree.settings.dialup.autoimei
            checkGotoHome(g_module.modsettings_enabled && checkObjectExist(g_PageUrlTree, "settings", "dialup", "autoimei"));
            break;
        ...
        case "wlanbasicsettings":
            ...
            checkGotoHome(g_module.wifi_enabled && checkObjectExist(g_PageUrlTree, "settings", "wlan", "wlanbasicsettings"));
            break;
        case "wlanadvanced":
            //g_PageUrlTree.settings.wlan.wlanadvanced
            checkGotoHome(g_module.wifi_enabled && checkObjectExist(g_PageUrlTree, "settings", "wlan", "wlanadvanced"));
            break;
        case "wlanmacfilter":
            //g_PageUrlTree.settings.wlan.wlanmacfilter
            checkGotoHome(g_module.wifi_enabled && checkObjectExist(g_PageUrlTree, "settings", "wlan", "wlanmacfilter"));
            break;
        case "wps":
        ...
        case "pincodeautovalidate":
            ...
            checkGotoHome(g_module.modsettings_enabled && checkObjectExist(g_PageUrlTree, "settings", "security", "pincodeautovalidate"));
            break;
        case "firewallswitch":
        ...
        case "deviceinformation":
            ...
            break;
        case "modsettings":
            //g_PageUrlTree.settings.system.modsettings
        ...
        case "dhcp":
            ...
            break;
        case "hosts":
            //g_PageUrlTree.settings.system.hosts
            checkGotoHome(g_module.modsettings_enabled && checkObjectExist(g_PageUrlTree, "settings", "system", "hosts"));
            break;
        case "watchcat":
            //g_PageUrlTree.settings.system.watchcat
            checkGotoHome(g_module.modsettings_enabled && checkObjectExist(g_PageUrlTree, "settings", "system", "watchcat"));
            break;
        case "diagnosis":
...
function setMainMenu() {
    ...
    if (current_href == 'pincoderequired' || current_href == 'pukrequired' || current_href == 'simlockrequired' || current_href == 'nocard' || current_href == 'connectionless' || current_href == 'opennewwindow') {
        ...
        $('#deviceinformation2').children().text('?');
        $('#antennapointing').children().text('📡');
        ...
        if (g_module.sms_enabled && MenuNumber < g_MainMenuNumber) {
            if(force_new_sms && force_new_ussd && webui_mode == 'old') {
        ...
        if (g_module.ussd_enabled && ((!force_new_sms || !force_new_ussd) && webui_mode == 'old') && MenuNumber < g_MainMenuNumber) {
            $('#ussd').children().text(ussd_label_ussd);
            MenuNumber++;
        } else if(g_module.ussd_enabled && ((!force_new_sms || !force_new_ussd) && webui_mode == 'old') && MenuNumber >= g_MainMenuNumber) {
            $('#menu_ussd').remove();
            MenuNumber++;
      ...
      /*  if (g_module.online_update_enabled && MenuNumber < g_MainMenuNumber) {
        ...
        if (checkObjectExist(g_PageUrlTree, "mods") && MenuNumber < g_MainMenuNumber) {
            $('#mods').children().text(system_label_mods);
            MenuNumber++;
        } else if(checkObjectExist(g_PageUrlTree, "mods") && MenuNumber >= g_MainMenuNumber) {
            $('#menu_mods').remove();
            MenuNumber++;
            Add_more_menu("mods","mods.html",system_label_mods);
        } else {
            $('#menu_mods').remove();
        }
        ...
        if (!$("#more_menu_ul").get(0)) {
            $('#menu_more').remove();
        } else {
            if(webui_mode != 'material') {
                $('#more').die('click');
...
function Add_more_menu(MenuID,MenuAddress,MenuLabel) {
    ...
    if (!$("#more_menu_ul").get(0)) {
        ...
        if(webui_mode == 'material') {
            moreHtml = "<ul id='more_menu_ul' class='mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect' for='more'>";
            moreHtml += "</ul>";
            $("#union_main_menu").after(moreHtml);
            if(typeof(componentHandler) != 'undefined')
                componentHandler.upgradeElement($('#more_menu_ul').get(0));
        } else {
    ...
    moreHtml += "    <a id='" + MenuID +"'" + (webui_mode == 'material' ? " class='mdl-menu__item'" : "") + ">";
...
function selectCur(obj) {
    $.each(obj, function(i, n) {
        ...
        if (typeof n == 'object') {
            ...
            if (n == current_href || ((force_new_sms && force_new_ussd && webui_mode == 'old') && current_href == 'ussd' && n == 'smsinbox')) {
                TOPMENU.nav.push(n);
                TOPMENU.have = true;
            }
        }
...
function showCur(noSubMenu) {
    if (noSubMenu) {
     ...
     if("update_local" == current_href){
        ...
        } else if(current_href == 'autoimei') {
            $('#dialup').addClass('click');
            var cName = ($('#autoimei_item').get(0)).className;
            if (cName.indexOf('nosub') > -1) {
                cName = cName.replace('nosub ', 'nosub');
            }
            $('#autoimei_item')[0].className = cName;
            $('#autoimei_item').addClass('subClick');
        } else if(current_href == 'watchcat') {
            $('#system').addClass('click');
            var cName = ($('#watchcat_item').get(0)).className;
            if (cName.indexOf('nosub') > -1) {
                cName = cName.replace('nosub ', 'nosub');
            }
            $('#watchcat_item')[0].className = cName;
            $('#watchcat_item').addClass('subClick');
		}else{
...
function showleftMenu() {
    ...
    if ($('.main_left').children().is('#settings_menu')) {
        $('.main_left').load('leftmenu.html #settings_menu', function() {
            ...
            //quicksetup
            if (checkLeftMenu(g_PageUrlTree.settings.quick_setup) && g_Driver_classify != 'hilink') {
            ...
            if (checkLeftMenu(g_PageUrlTree.settings.dialup)) {
                ...
                if (g_module.modsettings_enabled && checkLeftMenu(g_PageUrlTree.settings.dialup.autoimei)) {
                    $('#label_autoimei').attr('title', dialup_label_autoimei);
                    $('#label_autoimei').text(dialup_label_autoimei);
                } else if(checkLeftMenu(g_PageUrlTree.settings.dialup.autoimei)) {
                    $('#autoimei_item').attr('data-disabled', true);
                    $('#autoimei_item a').attr('href', 'javascript:addWebUIInitDialog();void(0);');
                    $('#label_autoimei').attr('title', dialup_label_autoimei);
                    $('#label_autoimei').text(dialup_label_autoimei);
                } else {
                    $('#autoimei_item').remove();
                }
            ...
            //wlan
            if (g_module.wifi_enabled && checkLeftMenu(g_PageUrlTree.settings.wlan)) {
            ...
            if (checkLeftMenu(g_PageUrlTree.settings.security)) {
                if (g_module.modsettings_enabled && checkLeftMenu(g_PageUrlTree.settings.security.pincodeautovalidate)) {
                ...
                } else if (checkLeftMenu(g_PageUrlTree.settings.security.pincodeautovalidate)) {
                    $('#pincodeautovalidate').attr('data-disabled', true);
                    $('#pincodeautovalidate a').attr('href', 'javascript:addWebUIInitDialog();void(0);');
                    $('#label_pin_auto_validation').attr('title', dialup_label_pin_auto_validation);
                    $('#label_pin_auto_validation').text(dialup_label_pin_auto_validation);
                } else {
            ...
            if (checkLeftMenu(g_PageUrlTree.settings.system)) {
                ...
                if (g_module.modsettings_enabled && checkLeftMenu(g_PageUrlTree.settings.system.hosts)) {
                    $('#label_hosts').attr('title', system_label_hosts);
                    $('#label_hosts').text(system_label_hosts);
                } else if(checkLeftMenu(g_PageUrlTree.settings.system.hosts)) {
                    $('#hosts').attr('data-disabled', true);
                    $('#hosts a').attr('href', 'javascript:addWebUIInitDialog();void(0);');
                    $('#label_hosts').attr('title', system_label_hosts);
                    $('#label_hosts').text(system_label_hosts);
                } else {
                    $('#hosts').remove();
                ...
                if (g_module.modsettings_enabled && checkLeftMenu(g_PageUrlTree.settings.system.watchcat)) {
                    $('#label_watchcat').attr('title', system_label_watchcat);
                    $('#label_watchcat').text(system_label_watchcat);
                } else if(checkLeftMenu(g_PageUrlTree.settings.system.watchcat)) {
                    $('#watchcat').attr('data-disabled', true);
                    $('#watchcat a').attr('href', 'javascript:addWebUIInitDialog();void(0);');
                    $('#label_watchcat').attr('title', system_label_watchcat);
                    $('#label_watchcat').text(system_label_watchcat);
                } else {
                    $('#watchcat').remove();
...
function setLangList() {
    ...
    if(webui_mode == 'material')
        $('#lang').removeClass('browser-default');
    else
        $('#lang').show();
...
function showFooter() {
    ...
    if (g_needFooter) {
        $('#footer').load('footer.html img, span, rightcorner', function() {
            ...
            $('#footer').prepend("<a id='pravacy_policy' class='clr_gray'>Project Mode</a>");
            ...
            getAjaxData('api/device/device-feature-switch', function($xml) {
                ...
                if (ret.type == 'response') {
                    ...
                    if(g_copyright_enabled == '1') {
                        $('#footer span').after("<span id='copyright_notice'><a href='javascript:window.open(\"/html/switchDebugMode.html\");void(0);' class='clr_gray'>Debug Mode</a></span>");
                        if(webui_mode == 'material') {
                            $('#copyright_notice').addClass('mdl-cell mdl-cell--2-col mdl-cell--1-col-phone mdl-typography--text-right');
                        }
                    }
                }
            });
            ...
            if(webui_mode == 'material') {
                $('#footer').replaceWith('<footer id="footer" class="mdl-grid">' + $('#footer').html() + '</div>');
                $('#pravacy_policy').addClass('mdl-cell mdl-cell--2-col mdl-cell--1-col-phone');
                $('#statement_main').addClass('mdl-cell mdl-cell--8-col-desktop mdl-cell--4-col-tablet mdl-cell--2-col-phone mdl-typography--text-center');
                $('img[src="../res/logo-footer.gif"]').prependTo('#statement_main');
            }
        });
    }
}
...
var factoryResetRequired = false;
var modsettings = {};
var user_config = {};
var webui_mode = 'new';
...
function showConnectInfo() {
    if(webui_mode == "new") {
        $('#center_box_main').css('marginLeft', 245);
        return
    }
...
$(document).ready( function() {
    ...
    $('.hotlinkstring').show();
    //load header,
    if(factoryResetRequired && current_href != 'factoryresetrequired' && current_href != 'pincoderequired' && current_href != 'pukrequired' && current_href != 'simlockrequired' && current_href != 'nocard'&& current_href != 'cradleDisconnected') {
        gotoPageWithoutHistory('factoryresetrequired.html');
        return;
    }
    if (g_setup_wizard_page == '1' && !factoryResetRequired && g_Driver_classify != 'hilink' && g_auto_update_enable == '1' && current_href != 'quicksetup' && current_href != 'pincoderequired' && current_href != 'pukrequired' && current_href != 'simlockrequired' && current_href != 'nocard'&& current_href != 'cradleDisconnected') {
        gotoPageWithoutHistory('quicksetup.html');
        return;
    }
    if (g_setup_wizard_page != '1' && !factoryResetRequired && g_default_password_status == 0 && current_href != 'modifypassword' && current_href != 'pincoderequired' && current_href != 'pukrequired' && current_href != 'simlockrequired' && current_href != 'nocard' && current_href != 'cradleDisconnected' && current_href != 'commend') {
        gotoPageWithoutHistory('modifypassword.html');
        return;
    }
    if (current_href == 'pincoderequired' || current_href == 'pukrequired' || current_href == 'simlockrequired' || current_href == 'nocard'  || current_href == 'cradleDisconnected'  || current_href == 'update_autorun'  || current_href == 'voicebusy' || current_href == 'opennewwindow' || current_href == 'factoryresetrequired') {
        loadHeaderRedirectPage();
    } else if(current_href != 'antennapointing') {
        loadHeader();
    }
    showConnectInfo();
    getSimPukTimes();
    ...
    */
    ...
    $('#deviceinformation2').live('click', function() {
        $("#deviceinformation2").attr('href','deviceinformation.html' );
    });
    $('#antennapointing').live('click', function() {
        $("#antennapointing").attr('href','antennapointing.html' );
    });
    ...
    $('#mods').live('click', function() {
        if(g_module.modsettings_enabled)
            $("#mods").attr('href','mods.html' );
        else
            $("#mods").attr('href','javascript:addWebUIInitDialog();void(0);' );
    });
...
function loadHeaderRedirectPage() {
    if($.trim($('#header').html()) == '' || $('#header').html() == null) {
        if(webui_mode == 'material') {
            $('.login_head_div.login_head_dim').replaceWith('<header id="header" class="mdl-layout__header"><div class="mdl-layout__header-row"><span class="mdl-layout-title">' + g_feature.title + '</span></div></header>');
            $('header').after(
            ...
            if (current_href == 'cradleDisconnected' || current_href == 'pincoderequired' || current_href == 'pukrequired'|| current_href == 'simlockrequired') {
                $('.mdl-layout__drawer > .mdl-layout-title').after("<nav class='help mdl-navigation'><span class='mdl-navigation__link'><select id='lang' class='browser-default' style='display:none'></select></span><a id=help_url class='mdl-navigation__link' href='#' target='_blank'><span id='help_span'></span></a><span class='username mdl-navigation__link'  id='username_span' style='display:none'></span><a id='logout_span' class='mdl-navigation__link logout' href='javascript:void(0);'></a></nav>");
            } else  if(current_href == 'update_autorun'){
                $('.mdl-layout__drawer > .mdl-layout-title').after("<nav class='help mdl-navigation'><span class='mdl-navigation__link'><select id='lang' class='browser-default' style='display:none'></select></span></nav>");
            }else {
                $('.mdl-layout__drawer > .mdl-layout-title').after("<nav class='language mdl-navigation'><span class='mdl-navigation__link'><select id='lang' class='browser-default' style='display:none'></select></span></nav>");
            }
            setLangList();
            showCurrentLanguage();
            if (g_needToLogin) {
            ...
            if(typeof(componentHandler) != 'undefined') {
                componentHandler.downgradeElements($('#all_content').get(0));
                componentHandler.upgradeElement($('#all_content').get(0));
            }
        } else {
...
function loadHeader () {
    ...
    if($.trim($('#header').html()) == '' || $('#header').html() == null) {
        if(webui_mode == 'material') {
            $('#header').replaceWith('<header id="header" class="mdl-layout__header"><div class="mdl-layout__header-row"><span class="mdl-layout-title">' + g_feature.title + '</span></div></header>');
            $.ajax({
                type: "GET",
                url: "header.html",
                success: function(data, textStatus, jqXHR) {
                    $('header').after(
                        '<div class="mdl-layout__drawer">' +
                            '<span class="mdl-layout-title">' +
                                g_feature.title +
                            '</span>' +
                            '<nav id="nav-mobile" class="mdl-navigation  mdl-layout--small-screen-only"></nav>' +
                            '<nav class="help mdl-navigation">' +
                                '<span class="mdl-navigation__link">' +
                                    '<select id="lang" class="browser-default" style="display:none">' +
                                    '</select>' +
                                '</span>' +
                                '<a id=help_url class="mdl-navigation__link" href="#" target="_blank">' +
                                    '<span id="help_span"></span>' +
                                '</a>' +
                                '<span class="mdl-navigation__link"  id="username_span" style="display:none">' +
                                '</span>' +
                                '<a id="logout_span" class="logout mdl-navigation__link" href="javascript:void(0);"></a>' +
                            '</nav>' +
                        '</div>'
                    );
                    ...
                    var union_main_menu = $(data).find('#union_main_menu > ul > li > a');
                    if(union_main_menu.get(0)) {
                        $('header .mdl-layout-title').after('<div class="mdl-layout-spacer"></div><nav id="union_main_menu" class="mdl-navigation  mdl-layout--large-screen-only"></nav>');
                        union_main_menu.addClass('mdl-navigation__link');
                        union_main_menu.each(function() {
                            if(!$(this).is('a[href]'))
                                $(this).attr('href', 'javascript:void(0);');
                        });
                        $('#union_main_menu').html(union_main_menu);
                        showCur(true);
                        setMainMenu();
                        union_main_menu = $('#union_main_menu').children();
                        union_main_menu.each(function() {
                            if($(this).children().html().length == 0)
                                $(this).remove();
                        });
                        union_main_menu = $('#union_main_menu').children();
                        var nav_mobile = union_main_menu.clone();
                        nav_mobile.each(function() {
                            $(this).attr('data-id', $(this).attr('id')).attr('id','nav-mobile-' + $(this).attr('id'));
                            if(!$(this).is('#nav-mobile-more')) {
                                $(this).click(function(e) {
                                    document.querySelector('#' + $(this).attr('data-id')).click();
                                    e.preventDefault();
                                });
                            }
                        });
                        $('#nav-mobile').html(nav_mobile);
                        var navMobileMoreHtml = "<ul id='nav_mobile_more_menu_ul' class='mdl-menu mdl-menu--bottom-left mdl-js-menu mdl-js-ripple-effect' for='nav-mobile-more'>";
                        navMobileMoreHtml += "</ul>";
                        $("#nav-mobile").after(navMobileMoreHtml);
                        var nav_mobile_more = $('#more_menu_ul').children().clone();
                        nav_mobile_more.each(function() {
                            $(this).attr('data-id', $(this).attr('id')).attr('id','nav-mobile-' + $(this).attr('id'));
                            $(this).children().attr('data-id', $(this).children().attr('id')).attr('id','nav-mobile-' + $(this).children().attr('id'));
                            $(this).children().click(function(e) {
                                document.querySelector('#' + $(this).attr('data-id')).click();
                                e.preventDefault();
                            });
                        });
                        $('#nav_mobile_more_menu_ul').html(nav_mobile_more);
                        if(typeof(componentHandler) != 'undefined')
                            componentHandler.upgradeElement($('#nav_mobile_more_menu_ul').get(0));
                    }
                    ...
                    var menu_id = '#deviceinformation2,#antennapointing,#statistic,#sms,#payment,#menu_connection_settings a,#ussd,#update,#mods,#settings,.nav01,.nav02,#profile_settings a,#pb,#stk, #wifi_turnOff_button,#sambasettings,#sambasharinguser,#dlna,#appmanagement,#update_local';
                    if(!g_module.sdcard_enabled) {
                        menu_id += ',#sharing';
                    }
                    if(!g_not_need_login_update){
                        menu_id += ',#update,.nav01';
                    }
            ...
            $('#header').load('header.html', function() {
                ...
                var menu_id = '#menu_deviceinformation,#menu_antennapointing,#menu_statistic,#menu_sms,#menu_payment,#menu_connection_settings a,#menu_ussd,#menu_update,#menu_mods,#menu_settings,.nav01,.nav02,#profile_settings a,#menu_pb,#menu_stk, #wifi_turnOff_button,#sambasettings,#sambasharinguser,#dlna,#menu_appmanagement,#update_local';
                if(!g_module.sdcard_enabled) {
...
function call_dialog(dialogTitle, content, button1_text, button1_id, button2_text, button2_id) {
    ...
    if(webui_mode == 'material') {
        document.querySelectorAll('.dialog').forEach(function(item, i, arr) {
            item.close();
        });
        $('.dialog').remove();

        var dialogHtml = '';
        dialogHtml += "<h4 class='mdl-dialog__title'>" + dialogTitle + "</h4>";
        dialogHtml += "<div class='mdl-dialog__content'>";
        dialogHtml += content;
        dialogHtml += '</div>';
        dialogHtml += "<div class='mdl-dialog__actions'>";

        dialogHtml += "    <a href='javascript:void(0);' id='"+button1_id+"' class='mdl-button'>" + button1_text + "</a>";

        if (button2_text != '' && button2_text != ' ' && button2_text != null) {
            dialogHtml += "    <a href='javascript:void(0);' id='" + button2_id + "' class='mdl-button'>" + button2_text + "</a>";
        }
        dialogHtml += '</div>';

        var dialog = document.createElement('dialog');
        dialog.className = 'dialog mdl-dialog';
        dialog.id = 'sms_dialog';
        dialog.style = 'width: auto;';
        dialog.innerHTML = dialogHtml;

        $('.body_bg').before(dialog);
        if (! dialog.showModal) {
...
function showloginDialog() {
    if(webui_mode == 'material') {
        document.querySelectorAll('.login_dialog').forEach(function(item, i, arr) {
            item.close();
        });
        $('.login_dialog').remove();

        var dialogHtml = '';
        dialogHtml += "<h4 class='mdl-dialog__title'>" + common_login + "</h4>";
        dialogHtml += "<div class='mdl-dialog__content'>";
        ...
        dialogHtml += '</div>';
        dialogHtml += "<div class='mdl-dialog__actions'>";
        dialogHtml += "      <a href='javascript:void(0);' id='pop_login' class='mdl-button'>"+common_login+"</a>";

        dialogHtml += "      <a href='javascript:void(0);' id='pop_Cancel' class='mdl-button'>"+common_cancel+"</a>";

        dialogHtml += '</div>';

        var dialog = document.createElement('dialog');
        dialog.className = 'login_dialog mdl-dialog';
        dialog.id = 'dialog';
        dialog.innerHTML = dialogHtml;
...
function showConfirmDialog(content, okFunc, cancelFunc, removeable,removeFunc) {
    ...
    if(webui_mode == 'material') {
        ...
        var dialogHtml = '';
        dialogHtml += "<h4 class='mdl-dialog__title'>"  + common_confirm + "</h4>";
        dialogHtml += "<div class='mdl-dialog__content'>";
        dialogHtml += content;
        dialogHtml += '</div>';
        dialogHtml += "<div class='mdl-dialog__actions'>";
        dialogHtml += "    <a href='javascript:void(0);' id='pop_confirm' class='mdl-button'>"+common_ok+"</a>";
...
function showInfoDialog(tipsContent, isWiFi, closeFunc, notAutoCloss) {
    ...
    if ($('.info_dialog').size() == 0) {
        ...
        if(webui_mode == 'material') {
            var dialogHtml = '';
            dialogHtml += "<h4 class='mdl-dialog__title'>" + common_note + "</h4>";
            dialogHtml += "<div class='mdl-dialog__content'>";
            dialogHtml += tipsContent;
            dialogHtml += '</div>';
            dialogHtml += "<div class='mdl-dialog__actions'>";
            dialogHtml += "    <a href='javascript:void(0);' id='pop_Cancel' class='mdl-button'>"+common_close+"</a>";
            dialogHtml += '</div>';

            var dialog = document.createElement('dialog');
            dialog.className = 'info_dialog mdl-dialog';
            dialog.innerHTML = dialogHtml;
...
function clearDialog() {
    if(webui_mode == 'material') {
        document.querySelectorAll('.dialog:not(#upload_dialog), .info_dialog, .login_dialog').forEach(function(item, i, arr) {
            item.close();
        });
    }
...
function getGlobalFeatures() {
    ...
    getConfigData('config/global/config.xml', function($xml) {
        ...
        g_feature.battery_enabled = g_feature.battery_enabled && G_MonitoringStatus.response.BatteryStatus != '';
        g_PageUrlTree = g_feature.menu;
        g_needToLogin = g_feature.login && isFirmwareSupportsModule('api/wlan/basic-settings');
        g_nedd_login = g_feature.login && isFirmwareSupportsModule('api/wlan/basic-settings');
        g_needFooter = g_feature.footer && (webui_mode == 'old' || webui_mode == 'material');
        g_MainMenuNumber = (force_new_sms && force_new_ussd && webui_mode == 'old') ? 6 : 7 /*g_feature.menu_number*/;
    ...
    getConfigData('config/ussd/prepaidussd.xml', function($xml) {
        var ret = xml2object($xml);
        if (ret.type == 'config') {
            if(typeof(g_PageUrlTree.ussd.prepaid.pre_fun_balanceInquiry) != 'undefined' && ret.config.USSD.BalanceInquiry.Items.Item.Command == '')
                delete g_PageUrlTree.ussd.prepaid.pre_fun_balanceInquiry;
        }
    }, {
        sync: true
    });
    ...
    getAjaxData('api/global/module-switch', function($xml) {
        ...
        if (ret.type == 'response') {
            ...
            g_module = {
                ...
                'sdcard_enabled' : g_moduleswitch.sdcard_enabled == '1' && isSdCardEnabled() ? true : false,
                'online_update_enabled' : g_moduleswitch.bbou_enabled == '1' ? true : false,
                'wifi_enabled' : g_moduleswitch.wifi_enabled == '1' && isFirmwareSupportsModule('api/wlan/basic-settings') ? true : false,
                ...
                'wifioffload_enable' : g_moduleswitch.wifioffload_enabled == '1' && isFirmwareSupportsModule('api/wlan/wifiprofile') ? true : false,
                ...
                'poweroff_enabled':g_moduleswitch.poweroff_enabled == '1' && g_feature.battery_enabled ? true : false,
...
function showWaitingDialog(tipTitle, tipContent, callback_func) {
    ...
    if(webui_mode == 'material') {
        var wait_div = "<h4 class='mdl-dialog__title'>" + tipTitle + "</h4>" + "<div class='mdl-dialog__content'>" + "<center><div class='preloader-wrapper big active'><div class='spinner-layer spinner-blue'><div class='circle-clipper left'><div class='circle'></div></div><div class='gap-patch'><div class='circle'></div></div><div class='circle-clipper right'><div class='circle'></div></div></div><div class='spinner-layer spinner-red'><div class='circle-clipper left'><div class='circle'></div></div><div class='gap-patch'><div class='circle'></div></div><div class='circle-clipper right'><div class='circle'></div></div></div><div class='spinner-layer spinner-yellow'><div class='circle-clipper left'><div class='circle'></div></div><div class='gap-patch'><div class='circle'></div></div><div class='circle-clipper right'><div class='circle'></div></div></div><div class='spinner-layer spinner-green'><div class='circle-clipper left'><div class='circle'></div></div><div class='gap-patch'><div class='circle'></div></div><div class='circle-clipper right'><div class='circle'></div></div></div></div><p>" + tipContent + '</p></center>' + '</div>' + "<div class='mdl-dialog__actions'><a href='javascript:void(0);' id='pop_Cancel' class='mdl-button'>"+common_close+"</a></div>";

        var dialog = document.createElement('dialog');
        dialog.id = 'wait_table';
        dialog.className = 'wait_table mdl-dialog';
        dialog.innerHTML = wait_div;
...
function closeWaitingDialog(callback_func) {
    ...
    if(webui_mode == 'material') {
        document.querySelectorAll('#wait_table').forEach(function(item, i, arr) {
            item.close();
        });
    }
    $('#wait_table').remove();
...
function qtps_signal() {
    ...
    if (rssi == "") {
        getConfigData('config/deviceinformation/add_param.xml', function($xml) {
            var ret = xml2object($xml);
            if (ret.type == 'config' && ret.config.rssi != undefined) {
    ...
    if (rsrp != "")
        current_qtps_signal += "<br>RSRP: " + rsrp;
    if (rsrq != "")
        current_qtps_signal += "<br>RSRQ : " + rsrq;
    if (sinr != "")
        current_qtps_signal += "<br>SINR: " + sinr;
    if (rscp != "")
        current_qtps_signal += "<br>RSCP: " + rscp;
    if (ecio != "")
        current_qtps_signal += "<br>Ec/Io: " + ecio;
    return current_qtps_signal;
}

function qtps_multissid() {
    var ssid_list_index = '';

    getAjaxData('api/wlan/multi-basic-settings', function($xml) {
        var ret = xml2object($xml);
        if (ret.type == 'response' && typeof(ret.response.Ssids.Ssid) != 'undefined') {
            for(var i = 0; i < ret.response.Ssids.Ssid.length; i++) {
                ssid_list_index += ret.response.Ssids.Ssid[i].WifiSsid + common_colon + ' ' + ret.response.Ssids.Ssid[i].WifiAssociatedStationNum + (i != (ret.response.Ssids.Ssid.length - 1) ? '<br>' : '');
            }
        }
    }, {
        sync: true
...
function getIconStatus() {
    ...
    if (header_ret != null && header_ret.type == 'response') {
            ...
            switch (header_icon_status.BatteryStatus) {
                ...
                case MACRO_BATTERY_STATUS_LOW:
                    if(g_coulometer_status == '1') {
                        ...
                        STATUS_BAR_ICON_STATUS.battery_tooltip_state = battery_recharging + ' (' + header_icon_status.BatteryPercent + '%)';
                        if (null != g_lastBatteryStatus && MACRO_BATTERY_STATUS_ELECT == g_lastBatteryStatus &&
                            last_battery_level != null && header_icon_status.BatteryLevel == last_battery_level) {
                            break;
                        }
                        switch (header_icon_status.BatteryLevel) {
                            case MACRO_BATTERY_LEVEL_ZERO:
                                $('#battery_gif').html("<img   src = '../res/battery_elect_0.gif' />");
                                break;
                            case MACRO_BATTERY_LEVEL_ONE:
                                $('#battery_gif').html("<img   src = '../res/battery_elect_25.gif' />");
                                break;
                            case MACRO_BATTERY_LEVEL_TWO:
                                $('#battery_gif').html("<img   src = '../res/battery_elect_50.gif' />");
                                break;
                            case MACRO_BATTERY_LEVEL_THREE:
                                $('#battery_gif').html("<img   src = '../res/battery_elect_75.gif' />");
                                break;
                            default:
                                $('#battery_gif').html("<img   src = '../res/battery_elect.gif' />");
                                break;
                        ...
                        $('#battery_gif').show();
                        last_battery_level = header_icon_status.BatteryLevel;
                    }
                    break;
        ...
        if( ( $.browser.msie && ($.browser.version == '6.0')) || g_WifiStatus!=header_icon_status.WifiStatus) {
            ...
            switch (header_icon_status.WifiStatus) {
                ...
                case MACRO_WIFI_ON:
                    ...
                    STATUS_BAR_ICON_STATUS.wifi_tooltip_state = qtps_multissid() ? qtps_multissid() : wlan_label_wlan_on;
                    break;
                default:
                    $('#wifi_gif').html("<img onload = 'fixPNG(this)' src = '../res/wifi_" + MACRO_WIFI_OFF + ".png' 0 0 no-repeat>");
                    STATUS_BAR_ICON_STATUS.wifi_tooltip_state = wlan_label_wlan_off;
        ...
        if((g_SimStatus != g_main_convergedStatus.SimState)||(g_hSimStatus != header_icon_status.SimStatus)) {
            if((webui_mode == 'new' && G_MonitoringStatus.response.WifiConnectionStatus != WIFI_CONNECTED) || webui_mode != 'new') {
            ...
            if (MACRO_PIN_REQUIRED == g_main_convergedStatus.SimState) {
                if((webui_mode == 'new' && G_MonitoringStatus.response.WifiConnectionStatus != WIFI_CONNECTED) || webui_mode != 'new') {
                    $('#sim_signal_gif').html("<img onload = 'fixPNG(this)' src = '../res/sim_disable" + (webui_mode == 'old' ? ".old" : "") + ".png' 0 0 no-repeat>");
                    $('#new_nav_signal_text').empty();
                    $('#status_img2').hide().empty();
                    STATUS_BAR_ICON_STATUS.sim_signal_tooltip_error = dialup_label_pin_code_required;
                }
                ...
                if((webui_mode == 'new' && G_MonitoringStatus.response.WifiConnectionStatus != WIFI_CONNECTED) || webui_mode != 'new') {
                    if (PUK_TIMES_ZERO == g_pin_status_SimPukTimes) {
                        ...
                        $('#new_nav_signal_text').empty();
                        $('#status_img2').hide().empty();
                        STATUS_BAR_ICON_STATUS.sim_signal_tooltip_error = dialup_help_puk_locked;
                    } else {
                        $('#sim_signal_gif').html("<img onload = 'fixPNG(this)' src = '../res/sim_disable" + (webui_mode == 'old' ? ".old" : "") + ".png' 0 0 no-repeat>");
                        $('#new_nav_signal_text').empty();
                        $('#status_img2').hide().empty();
                        STATUS_BAR_ICON_STATUS.sim_signal_tooltip_error = dialup_label_puk_code_required;
                    }
                }
                ...
                if((webui_mode == 'new' && G_MonitoringStatus.response.WifiConnectionStatus != WIFI_CONNECTED) || webui_mode != 'new') {
                    ...
                    $('#new_nav_signal_text').empty();
                    $('#status_img2').hide().empty();
                    STATUS_BAR_ICON_STATUS.sim_signal_tooltip_error = dialup_label_sim_invalid;
                }
                ...
                sign_enable = 1;
                STATUS_BAR_ICON_STATUS.sim_signal_tooltip_error = '';
            }
        }
        ...
        if(sign_enable) {
            ...
            STATUS_BAR_ICON_STATUS.sim_signal_tooltip_state = STATUS_BAR_ICON_STATUS.sim_signal_tooltip_error;
        }

        if(webui_mode == 'new') {
            if(header_icon_status.WifiStatus == MACRO_WIFI_ON && qtps_multissid())
                STATUS_BAR_ICON_STATUS.sim_signal_tooltip_state += '<br>' + qtps_multissid();
            if(g_feature.battery_enabled) {
                switch(header_icon_status.BatteryStatus) {
                    case MACRO_BATTERY_STATUS_LOW:
                    case MACRO_BATTERY_STATUS_ELECT:
                        STATUS_BAR_ICON_STATUS.sim_signal_tooltip_state += '<br>' + battery_recharging + ' (' + header_icon_status.BatteryPercent + '%)';
                        break;
                    case MACRO_BATTERY_STATUS_NOBATTERY:
                        break;
                    case MACRO_BATTERY_STATUS_NORMAL:
                    default:
                        STATUS_BAR_ICON_STATUS.sim_signal_tooltip_state += '<br>' + common_battery + ': ' + header_icon_status.BatteryPercent + '%';
                        break;
                }
            }
        }
        ...
        if (G_MonitoringStatus.response.WifiConnectionStatus == WIFI_CONNECTED) {
            $('#new_nav_signal_text').text(wlan_lable_wifi);
            $('#status_img2').text(wlan_lable_wifi).show();
        }

        if(sign_enable) {
            if (G_MonitoringStatus.response.WifiConnectionStatus != WIFI_CONNECTED) {
                $('#new_nav_signal_text').text(g_net_mode);
                if(g_net_mode != dialup_label_no_service)
                    $('#status_img2').text(g_net_mode).show();
                else
                    $('#status_img2').hide().text(g_net_mode);
            }
        }

        if(sign_enable) {
            if(($.browser.msie && ($.browser.version == '6.0')) || g_SignalStrength!=header_icon_status.SignalStrength && (SIMCARD_OK == sign_enable) && ((webui_mode == 'new' && G_MonitoringStatus.response.WifiConnectionStatus != WIFI_CONNECTED) || webui_mode != 'new') ) {
                ...
                switch (header_icon_status.SignalStrength) {
                    ...
                    case MACRO_EVDO_LEVEL_THREE:
                        $('#sim_signal_gif').html("<img onload = 'fixPNG(this)' src = '../res/signal_" + header_icon_status.SignalStrength + (webui_mode == 'old' ? ".old" : "") + ".png'0 0 no-repeat>");
                        break;
                    ...
                    case MACRO_EVDO_LEVEL_FIVE:
                        ...
                        break;
                    default:
                        $('#sim_signal_gif').html("<img onload = 'fixPNG(this)' src = '../res/signal_" + MACRO_EVDO_LEVEL_ZERO + (webui_mode == 'old' ? ".old" : "") + ".png'0 0 no-repeat>");
                        break;
                }
            }
        ...
        && station_ret.type == "response" ) {
            ...
            if( ( $.browser.msie && ($.browser.version == '6.0')) || typeof ($("#station_gif").html())=="undefined"||$("#station_gif").html().indexOf("station_" + apSignal + ".png")<0) {
                ...
                if(webui_mode == 'new') {
                    g_SimStatus = -11111;
                    g_hSimStatus = -11111;
                    g_SignalStrength = -11111;
                    if(apSignal == 0)
                        $('#sim_signal_gif').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" width="34" height="34" viewBox="0 0 18 18" style="margin: 8px 5px 8px 10px;"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/></svg>');
                    else if(apSignal == 1)
                        $('#sim_signal_gif').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" width="34" height="34" viewBox="0 0 18 18" style="margin: 8px 5px 8px 10px;"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/><path d="M5.24 11.31L9 16l3.76-4.68C12.57 11.17 11.14 10 9 10c-2.14 0-3.57 1.17-3.76 1.31z"/></svg>');
                    else if(apSignal == 2)
                        $('#sim_signal_gif').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" width="34" height="34" viewBox="0 0 18 18" style="margin: 8px 5px 8px 10px;"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/><path d="M3.98 9.75l5.01 6.24L9 16v-.01l5.01-6.24C13.76 9.56 11.85 8 9 8 6.15 8 4.24 9.56 3.98 9.75z"/></svg>');
                    else if(apSignal == 3)
                        $('#sim_signal_gif').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" width="34" height="34" viewBox="0 0 18 18" style="margin: 8px 5px 8px 10px;"><path fill-opacity=".3" d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/><path d="M2.73 8.19L9 15.99V16l.01-.01 6.27-7.8C14.96 7.94 12.56 6 9 6S3.04 7.94 2.73 8.19z"/></svg>');
                    else if(apSignal == 4)
                        $('#sim_signal_gif').html('<svg xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" width="34" height="34" viewBox="0 0 18 18" style="margin: 8px 5px 8px 10px;"><path d="M9.01 15.99l8.8-10.96C17.47 4.77 14.08 2 9 2S.53 4.77.19 5.03l8.8 10.96h.02z"/></svg>');
                }
            }
            $("#station_gif").show();
...
function showPWRemindDialog(destnation,callback) {
    if(webui_mode == 'material') {
        ...
        dialogHtml += "<h4 class='mdl-dialog__title'>" + common_note + "</h4>";
        dialogHtml += "<div class='mdl-dialog__content'>";
        dialogHtml += "    <p>" + IDS_psw_login_remind  + "</p>";
        dialogHtml += "    <label class='mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect' for='check_pass_remind'><input type='checkbox'  id='check_pass_remind' class='mdl-checkbox__input' /><span class='mdl-checkbox__label'>" + IDS_psw_modify_remind + "</span></label>";
        dialogHtml += "</div>";
        dialogHtml += "<div class='mdl-dialog__actions'>";
        dialogHtml += "    <a href='javascript:void(0);' id='psw_confirm' class='mdl-button'>"+IDS_common_modify+"</a>";
        dialogHtml += "    <a href='javascript:void(0);' id='psw_Cancel' class='mdl-button'>"+IDS_common_wait+"</a>";
        dialogHtml += "</div>";
        ...
        if(typeof(componentHandler) != 'undefined')
            componentHandler.upgradeElement(dialog.querySelector('label[for="check_pass_remind"]'));
...
function showDialog(common_title,content,button1_text,button1_Id,button2_text,button2_Id) {
    if(webui_mode == 'material') {
        ...
        var dialogHtml = '';
        dialogHtml += "<h4 class='mdl-dialog__title'>" + common_title + "</h4>";
        dialogHtml += "<div class='mdl-dialog__content'>";
        dialogHtml += content;
        dialogHtml += '</div>';
        dialogHtml += "<div class='mdl-dialog__actions'>";
...
function creatHeader() {
                ...
                <ul class="submenu">\
                    <li>\
                        <div id="new_nav_menu_home">\
                            ...
                            <div class="new_nav_menu_bg"><img src="../res/ic_home_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_deviceinformation">\
                            <a href="deviceinformation.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_info_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_antennapointing">\
                            <a href="antennapointing.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_settings_input_antenna_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_statistics">\
                            <a href="statistic.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_data_usage_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <!--<li>\
                    ...
                    <li>\
                        <div id="new_nav_menu_mods">\
                            <a href="mods.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_build_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_setting">\
                            <a href="mobileconnection.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_settings_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_sms">\
                            <a href="smsinbox.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_sms_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_pb">\
                            <a href="phonebook.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_contacts_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_ussd">\
                            <a href="ussd.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_perm_phone_msg_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_sharing">\
                            <a href="sdcardsharing.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_share_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_stk">\
                            <a href="stk.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_sim_card_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <li>\
                        <div id="new_nav_menu_appmanagement">\
                            <a href="appmanagement.html"></a>\
                            <div class="new_nav_menu_bg"><img src="../res/ic_get_app_7d7d7d_36px.svg" width="36" height="36"/></div>\
                        </div>\
                    </li>\
                    <!--<li>\
    ...
    if(!g_module.modsettings_enabled) {
        $('#new_nav_menu_mods a').attr('href', 'javascript:addWebUIInitDialog();void(0);');
    }

    if(!checkObjectExist(g_PageUrlTree, "mods")) {
        $('#new_nav_menu_mods').parent().remove();
    }

    if(!g_module.pb_enabled) {
        $('#new_nav_menu_pb').parent().remove();
    }

    if((webui_mode == 'new' && !force_old_sms) && (webui_mode == 'new' && !force_old_ussd)) {
        $('#new_nav_menu_ussd').parent().remove();
    }

    if(!g_module.sdcard_enabled) {
        $('#new_nav_menu_sharing').parent().remove();
    }

    if(!g_module.stk_enabled) {
        $('#new_nav_menu_stk').parent().remove();
    }
...
function creatFooter() {
    ...
    $('#new_footer').prepend("<a id='pravacy_policy' class='clr_gray'>Project Mode</a>");
    $('#pravacy_policy').attr("href","javascript:window.open('/html/switchProjectMode.html');void(0);");
    // setPrivacyPolicyUrl('#pravacy_policy');
...
function setMainText() {
    ...
    $('#new_nav_menu_deviceinformation a').text(IDS_system_logs_information);
    $('#new_nav_menu_antennapointing a').text(system_label_antenna);
    $('#new_nav_menu_mods a').text(system_label_mods);
	...
	$('#new_nav_menu_sharing a').text(sharing_label_sharing);
    $('#new_nav_menu_stk a').text(stk_label_stk);
    $('#new_nav_menu_appmanagement a').text(IDS_APP_MANAGEMENT);
   ...
   /* var g_network_action = {
    ...
    $('#new_switch_bg').live('click', function() {
        ...
        if($(this).hasClass("new_switch_bg_on")) {
            m_dialup_action.Action = '0';
            g_network_action.dataswitch = '0';
            m_handover_setting.Handover = '0';
        } else {
            m_dialup_action.Action = '1';
            g_network_action.dataswitch = '1';
            m_handover_setting.Handover = '2';
        }
    ...
    if(isFirmwareSupportsModule('api/user/state-login')) {
...
function setNavData() {
   ...
   /* if(G_MonitoringStatus.response.WifiStatus == 1) {
    ...
    getAjaxData('api/device/information', function($xml) {
        var ret = xml2object($xml);
        if (ret.type == 'response') {
            if(ret.response.Msisdn != mynumber) {
                if (typeof(ret.response.Msisdn) != 'undefined' && ret.response.Msisdn != '') {
                    $('#new_phone_number').html('<center><table style="position: relative;top: -15px"><tr><td><img src="../res/new_header_nav_user.png" width="64" height="64"></td><td><span style="font-size: 13px;font-weight: bold;">' + system_label_my_number + common_colon + '</span><br><span style="font-size: 12px;">' + ret.response.Msisdn + '</span></td></tr></table></center>');
                } else {
                    $('#new_phone_number').html('');
                }
...
$(document).ready(function () {
    ...
    if(webui_mode == 'material') {
        $('#all_content script').remove();
        $('#all_content').addClass('mdl-layout mdl-js-layout mdl-layout--fixed-header')
            .prepend($('#login_wrapper, #wrapper').detach().find('.login_wrapper_dim.login_wrapper_filter > *, .wrapper_dim.wrapper_filter > *'));
        if($('.center_box').get(0)) {
            $('.center_box').before('<main class="mdl-layout__content"><div class="mdl-grid"><div class="mdl-cell mdl-cell--12-col-desktop mdl-cell--8-col-tablet mdl-cell--4-col-phone"></div></div></main>');
            $('main > .mdl-grid > .mdl-cell').html($('.center_box, .notification, .login_box_info, .hot_links, #ad_div, #cbs_div').detach());
            $('.center_box').addClass('mdl-card mdl-shadow--2dp').css('width', '100%');
            $('#center_box_main').addClass('mdl-card__supporting-text mdl-grid').removeAttr('style');
            $('#status_info').addClass('mdl-cell mdl-cell--6-col-desktop mdl-cell--4-col-tablet mdl-cell--2-col-phone');
            $('#status_img').replaceWith('<div id="status_img_container" class="mdl-cell mdl-cell--6-col-desktop mdl-cell--4-col-tablet mdl-cell--2-col-phone"><div id="status_img" class="mdl-typography--text-center" style="float: right;">' + $('#status_img').html() + '</div></div>');
            $('#index_plmn_name').parent().replaceWith('<h4><p id="' + $('#index_plmn_name').attr('id') + '">' + $('#index_plmn_name').html() + '</p></h4>');
        } else {
            $($('.content, .stk_content, .content_box').get(0)).before('<main class="mdl-layout__content"></main>');
            $('.content, .stk_content, .content_box').each(function() {
                var contentChildren = $(this).children('*').detach();
                $('main').append($(this).detach());
                $(this).addClass('mdl-grid');
                $(this).html('<div class="mdl-cell mdl-cell--12-col-desktop mdl-cell--8-col-tablet mdl-cell--4-col-phone"></div>');
                $(this).children('.mdl-cell').html(contentChildren);
            });
        }
        $('main').append($('#footer').detach());
        $($('.agile_station').get(0)).before('<div id="agile_station_card" class="mdl-card mdl-shadow--2dp" style="width: 100%;"><div class="mdl-card__supporting-text"></div></div>');
        $('#agile_station_card > .mdl-card__supporting-text').html($('.agile_station').detach());
        $($('.agile_station_1').get(0)).before('<div id="agile_station_1_card" class="mdl-card mdl-shadow--2dp" style="width: 100%;"><div class="mdl-card__supporting-text"></div></div>');
        $('#agile_station_1_card > .mdl-card__supporting-text').html($('.agile_station_1').detach());
        $('.agile_statistics_table, .statistics_mobile_table').attr('width', '100%');
        $('.agile_station h2').each(function() {
            $(this).children('script').remove();
            $(this).parent().children('table').prepend('<thead><th colspan="2"><h5>' + $(this).html() + '</h5></th></thead>');
            $(this).remove();
        });
        $('#button_set_package').css({
            marginLeft: '50%',
            transform: 'translateX(-50%)'
        });
        $('.agile_lgtab').hide();
        $('.note > script').remove();
        $('#agile_station_card .mdl-card__supporting-text').append($('.agile_set_package.agile_station_button').detach()).append($('.traffic_status').detach());
        $('.note').html('<p>' + $('.note').html() + '<hr></p>');
        $('#span_button_clear_history').css('float', 'right').after('<br>');
        $('#clear_history table, #clear_history td').removeAttr('width').removeAttr('height');
        $('#clear_history table').attr('width', '100%');
        $('#clear_history table').attr('align', 'right');
        $('#clear_history table td').addClass('mdl-typography--text-right');
        $('.content_right, .content_right_sms, .content_right_pb').addClass('mdl-card mdl-shadow--2dp').css('width', '100%');
        var stkCard;
        if($('.stk_main_content').get(0)) {
            stkCard = document.createElement('div');
            stkCard.id = 'stk_content_card';
            stkCard.className = 'mdl-card mdl-shadow--2dp';
            stkCard.style = 'width: 100%';
            $('.stk_content > .mdl-cell').append(stkCard);
        }
        $('.main_content, .sms_main_content, .pb_main_content, .main_content_special, .stk_main_content').addClass('mdl-card__supporting-text');
        if($('.stk_main_content').get(0)) {
            $(stkCard).append($('.stk_main_content').detach());
        }
        $('table').attr('width', '100%');
        $('.maintitle > h1, .status_title.clr_blue_title_h2 h2:first, .card > :first-child > h1').each(function() {
            $(this).children('script').remove();
            $(this).replaceWith('<span id="' + $(this).attr('id') + '">' + $(this).html() +'</span>');
        });
        $('.maintitle, .status_title.clr_blue_title_h2:first, .card > :first-child').each(function() {
            $(this).prependTo('#agile_station_card, .content_right, .content_right_sms, .content_right_pb, #stk_content_card').replaceWith('<div class="mdl-card__title"><h2 class="mdl-card__title-text">' + $(this).html() + '</h2></div>');
        });
        $('h1').each(function() {
            $(this).children('script').remove();
            $(this).replaceWith('<h4 id="' + $(this).attr('id') + '" class="mdl-color-text--accent">' + $(this).html() + '</h4><hr>');
        });
        $('.main_content h2, .main_content_special h2').each(function() {
            $(this).children('script').remove();
            $(this).replaceWith('<h5 id="' + $(this).attr('id') + '">' + $(this).html() + '</h5>');
        });
        $('#networksearch').removeAttr('style');
        $('.main_content td').removeAttr('width').removeAttr('height');
        $('#stk_back_page').parent().parent().each(function() {
            $(this).replaceWith('<center id="' + $(this).attr('id') + '" class="' + $(this).attr('id') + '">' + $(this).html() + '</center>');
        });
        $('td > label, th > label').each(function() {
            $(this).children('script').remove();
            $(this).replaceWith("<span id=" + $(this).attr('id') + ">" + $(this).html() + "</span>");
        });
        $('#pb_img_search').hide();
        ...
        $.fn.val = function() {
            ...
            if(typeof($.fn.material_select) == 'function' && arguments.length && typeof(this.get(0)) != 'undefined' && this.get(0).tagName.toLowerCase() == 'select') {
                this.material_select();
                this.parent().removeClass(this.attr('class').replace(/(\\s|^)initialized(\\s|$)/g, ''));
            }
            return ret;
        }
        var jQueryAttr = $.attr;
        $.attr = function() {
            var ret = jQueryAttr.apply(this, arguments);
            if(arguments[0].parentNode && arguments.length > 2) {
                if(typeof(arguments[0].parentNode.MaterialRadio) != 'undefined') {
                    //arguments[0].parentNode.MaterialRadio.checkToggleState();
                    if(arguments[1] == 'checked' && arguments[2] === false)
                        arguments[0].parentNode.MaterialRadio.uncheck();
                    else if(arguments[1] == 'checked')
                        arguments[0].parentNode.MaterialRadio.check();
                    arguments[0].parentNode.MaterialRadio.checkDisabled();
                } else if(typeof(arguments[0].parentNode.MaterialCheckbox) != 'undefined') {
                    //arguments[0].parentNode.MaterialCheckbox.checkToggleState();
                    if(arguments[1] == 'checked' && arguments[2] === false)
                        arguments[0].parentNode.MaterialCheckbox.uncheck();
                    else if(arguments[1] == 'checked')
                        arguments[0].parentNode.MaterialCheckbox.check();
                    arguments[0].parentNode.MaterialCheckbox.checkDisabled();
                }
            }
            return ret;
        }
        var jQueryRemoveAttr = $.removeAttr;
        $.removeAttr = function() {
            var ret = jQueryRemoveAttr.apply(this, arguments);
            if(arguments[0].parentNode && arguments.length > 1) {
                if(typeof(arguments[0].parentNode.MaterialRadio) != 'undefined') {
                    arguments[0].parentNode.MaterialRadio.checkToggleState();
                    arguments[0].parentNode.MaterialRadio.checkDisabled();
                } else if(typeof(arguments[0].parentNode.MaterialCheckbox) != 'undefined') {
...
function setNetConnect_New ()
{
    ...
    g_isPostAjax = true;
    if(G_MonitoringStatus.response.WifiConnectionStatus == WIFI_CONNECTED) {
        var dialup_xml = object2xml('request', m_handover_setting);
        saveAjaxData('api/wlan/handover-setting', dialup_xml, function($xml) {
            var dialup_ret = xml2object($xml);
            g_isPostAjax = false;
            if (isAjaxReturnOK(dialup_ret)) {
                if('0' == m_handover_setting.Handover) {
                    $('#new_switch_bg').removeClass('new_switch_bg_on');
                } else {
                    $('#new_switch_bg').addClass('new_switch_bg_on');
                }
    ...
    } else if(connectMode == '1' && networkAction.dataswitch == '1') {
        var dialup_xml = object2xml('request', m_dialup_action);
        saveAjaxData('api/dialup/dial', dialup_xml, function($xml) {
            var dialup_ret = xml2object($xml);
            g_isPostAjax = false;
            if (isAjaxReturnOK(dialup_ret)) {
                if('0' == m_dialup_action.Action) {
                    $('#new_switch_bg').removeClass('new_switch_bg_on');
                } else {
                    $('#new_switch_bg').addClass('new_switch_bg_on');
                }
...
function showRestoreDialog() {
    if(webui_mode == 'material') {
        ...
        var dialogHtml = '';
        dialogHtml += "<h4 class='mdl-dialog__title'>" + common_restore + "</h4>";
        dialogHtml += "<div class='mdl-dialog__content'>";
        dialogHtml += "    <div class='login'>";
        ...
        dialogHtml += '</div>';
        dialogHtml += "<div class='mdl-dialog__actions'>";
        dialogHtml += "    <a href='javascript:void(0);' id='pop_restore' class='mdl-button'>"+common_restore+"</a>";
...
function GetPLMN_main() {
    ...
    (g_module.cradle_enabled && G_cradleStationStatus.cradlestatus == CRADLE_NETLINE_EXIST && G_cradleStationStatus.connectionmode != CRADLELANONLY && (CRADLEAUTOMODE != G_cradleStationStatus.connectionmode))))
        ...
        getAjaxData('operator.cgi', function($xml) {
    ...
    else if (connect_type == CURRENT_NETWORK_NO_SERVICE ||
    G_MonitoringStatus.response.CurrentServiceDomain == SERVICE_DOMAIN_NO_SERVICE ||
    G_MonitoringStatus.response.ServiceStatus != SERVICE_STATUS_AVAIABLE
    ) {
        plmn_name = dialup_label_no_service;
    }
    if (G_MonitoringStatus.response.WifiConnectionStatus == WIFI_CONNECTED) {
        plmn_name = G_StationStatus.response.NetworkName;
        ...
        plmn_name = IDS_plmn_label_wx;
    }
    $('#index_plmn_name_main').text(plmn_name);
    $('#new_switch_title').text(plmn_name);
}

var last_battery_level = null;
var mynumber = undefined;

function materializeInputs() {
    var radio = document.querySelectorAll('input[type="radio"]:not(.mdl-radio__button)');
    for (var i = 0; i < radio.length; i++) {
        $(radio[i]).parent().find('span script').remove();
        if (typeof($(radio[i]).attr('id')) == 'undefined')
            $(radio[i]).attr('id', $(radio[i]).attr('name') + '_' + i);
        $(radio[i]).addClass('mdl-radio__button');
        $(radio[i]).after(
            '<label class="mdl-radio mdl-js-radio mdl-js-ripple-effect" for="' + $(radio[i]).attr('id') + '">' +
                (radio[i].nextSibling && radio[i].nextSibling.nextSibling ?
                '<span class="mdl-radio__label">' +
                    $(radio[i].nextSibling.nextSibling).detach().text() +
                '</span>' :
                '<span class="mdl-radio__label"></span>') +
            '</label>'
        );
        $(radio[i]).next().prepend($(radio[i]).detach());
        if(typeof(componentHandler) != 'undefined')
            componentHandler.upgradeElement(radio[i].parentNode);
    }
    var checkbox = document.querySelectorAll('input[type="checkbox"]:not(.mdl-checkbox__input)');
    for (var i = 0; i < checkbox.length; i++) {
        $(checkbox[i]).parent().find('span script').remove();
        if (typeof($(checkbox[i]).attr('id')) == 'undefined')
            $(checkbox[i]).attr('id', $(checkbox[i]).attr('name') + '_' + i);
        $(checkbox[i]).addClass('mdl-checkbox__input');
        $(checkbox[i]).after(
            '<label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="' + $(checkbox[i]).attr('id') + '">' +
                (checkbox[i].nextSibling && checkbox[i].nextSibling.nextSibling ?
                '<span class="mdl-checkbox__label">' +
                    $(checkbox[i].nextSibling.nextSibling).detach().text() +
                '</span>' :
                '<span class="mdl-checkbox__label"></span>') +
            '</label>'
        );
        $(checkbox[i]).next().prepend($(checkbox[i]).detach());
        if(typeof(componentHandler) != 'undefined')
            componentHandler.upgradeElement(checkbox[i].parentNode);
    }
    $('input[type="file"]:not(.file_input_file)').each(function() {
        $(this).addClass('file_input_file');
        $(this).after(
            '<div class="file_input_div">' +
                '<div class="file_input">' +
                    ...
                    '</label>' +
                '</div>' +
                '<div class="file_input_text_div mdl-textfield mdl-js-textfield">' +
                    '<input class="file_input_text mdl-textfield__input browser-default" type="text" disabled readonly />' +
                    '<label class="mdl-textfield__label"></label>' +
                '</div>' +
            '</div>'
        );
        $(this).appendTo($(this).next().find('.image_input_button'));
        if(typeof(componentHandler) != 'undefined')
            componentHandler.upgradeElement(this.parentNode);
    });
    $('select > option > script').remove();
    ...
    $('input[type="text"]:not(.mdl-textfield__input, .select-dropdown), textarea:not(.mdl-textfield__input)').each(function() {
        $(this).addClass('mdl-textfield__input browser-default');
        $(this).after(
    ...
    $('pre').each(function() {
        $(this).replaceWith('<p id="' + $(this).attr('id') + '" class="' + $(this).attr('class') + '">' + $(this).html() + '</p>');
    });
}

function materializeButtons() {
    $('input[type="button"]').each(function() {
        $(this).replaceWith('<a href="javascript:void(0);" id="' + $(this).attr('id') + '" class="' + $(this).attr('class') + '" style="' + $(this).attr('style') + '">' + $(this).attr('value') + '</a>');
    });
    $('.mdl-card__supporting-text .apply_button').each(function() {
        $(this).parents('.mdl-card').append('<div class="mdl-card__actions mdl-card--border"></div>');
        $(this).parents('.mdl-card').children('.mdl-card__actions').append($(this).detach());
    });
    $('.apply_button, .ussd_right_concent, .align_right, pb_contact_right_bottom').addClass('mdl-typography--text-right');
    $('.button_dialog').addClass('mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--accent');
    $('.button_dialog:not(".apply_button .button_dialog")').addClass('mdl-button--raised');
    $('.disable_btn').addClass('mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--accent').attr('disabled', '');
    $('.disable_btn:not(".apply_button .disable_btn")').addClass('mdl-button--raised');
    $('.button_dialog, .disable_btn').each(function() {
        if(typeof(componentHandler) != 'undefined')
            componentHandler.upgradeElement(this);
    });
}

function materializeLeftMenu() {
    if ($('.main_left').get(0)) {
        if(!$('#all_content').is('.mdl-layout--fixed-drawer')) {
            $('#all_content').addClass('mdl-layout--fixed-drawer');
            if(typeof(componentHandler) != 'undefined')
                componentHandler.upgradeElement($('#all_content').get(0));
        }
        if(!$('#nav-leftmenu').get(0))
            $('#nav-mobile').after('<nav id="nav-leftmenu" class="mdl-navigation"></nav>');
    }
    if ($('.main_left > div > ul').get(0) && $('#nav-leftmenu').is(':empty')) {
        $('#nav-leftmenu').empty();
        $('.main_left > div > ul').each(function() {
            if(!$(this).is(':first-child')) {
                $(this).children('*').appendTo($(this).parent().children('ul:first'));
                $(this).remove();
            }
        });
        $('.main_left > div > ul p').each(function() {
            $(this).replaceWith('<span id="' + $(this).attr('id') + '" class="' + $(this).attr('class') + '">' + $(this).html() + '</span>');
        });
        $('.main_left > div > ul > li').each(function() {
            if(!$(this).find('a').get(0))
                $(this).html('<a href="javascript:void(0);">' + $(this).html() + '</a>');
            if($(this).hasClass('sub')) {
                var rootstyle = $(this).children('.rootstyle');
                var myUl = rootstyle.next();
                $('#nav-leftmenu').append(rootstyle.addClass('mdl-navigation__link').detach());
                myUl.attr('for', rootstyle.attr('id')).addClass('mdl-menu mdl-menu--bottom-left mdl-js-menu mdl-js-ripple-effect');
                myUl.children('li').children('a').addClass('mdl-menu__item').removeAttr('style');
                rootstyle.after(myUl.detach());
                rootstyle.replaceWith('<a href="javascript:void(0);" class="' + rootstyle.attr('class') + '" id="' + rootstyle.attr('id') + '">' + rootstyle.html() + '</a>');
                if(typeof(componentHandler) != 'undefined')
                    componentHandler.upgradeElement(myUl.get(0));
            } else {
                $('#nav-leftmenu').append($(this).children('a').addClass('mdl-navigation__link').removeAttr('style').detach());
            }
        });
        $('.main_left > div > ul').remove();
    }
}
...
function isFirmwareSupportsModule(url) {
	...
	getAjaxData(url, function($xml) {
		...
		if (ret.type == 'response') {
			...
			if (ret.error.code != ERROR_SYSTEM_NO_SUPPORT) {
				support = true;
			}
		}
	}, {
...
function isSdCardEnabled() {
	var SD_STATUS_DISABLE = '0';
	var sdcard = false;
	getAjaxData('api/sdcard/sdcard', function($xml) {
		var ret = xml2object($xml);
		if (ret.type == 'response') {
			if(ret.response.sdcard.SDCardStatus != SD_STATUS_DISABLE) {
				sdcard = true;
			}
		}
	}, {
...
function isFactoryResetRequired() {
	var need = false;
	getAjaxData('factoryreset.cgi', function($xml) {
		var ret = xml2object($xml);
...
function getUserConfig() {
	var config = {};
	getConfigData('user_config.data', function($xml) {
		var ret = xml2object($xml);
