...
function agile_set_month_volue(multi_obj, mobile_flag, wlan_flag) {
    ...
    $('#mobile_month_volume').html(getTrafficInfo2(parseInt(multi_obj.CurrentMonthDownload, 10) + parseInt(multi_obj.CurrentMonthUpload, 10)));
    $('#mobile_month_duration').html(getCurrentTime(multi_obj.MonthDuration));
    ...
    if (mobile_flag) {
        ...
        if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
            $('#month_total_value').html(g_data_unit + ' ' + g_monthly_info_mobile.data_limit);
        } else {
        ...
        if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
            $('#month_total_value').html(g_data_unit + ' ' + g_monthly_info_wlan.data_limit);
        } else {
...
function agile_mobile_set() {
    ...
    var action = {
        ...
        SetMonthData: parseInt(DataVolume, 10) == MACRO_NUMBER_FALSE ? MACRO_NUMBER_FALSE : MACRO_NUMBER_TRUE
    };
    ...
    saveAjaxData('api/monitoring/start_date', action_xml, function($xml) {
        ...
        if(parseInt(DataVolume, 10) == MACRO_NUMBER_FALSE) {
            g_set_data_package = MACRO_NUMBER_FALSE;
        } else {
...
$(document).ready( function() {
    ...
    if(force_both_statistic) {
        ...
        if((webui_mode == 'new' && !force_old_statistic) || (webui_mode == 'old' && force_new_statistic) || (webui_mode == 'material' && force_new_statistic)) {
            $('#new_content').show();
            $('#old_content').hide();
        } else {
            if((webui_mode == 'old' && !force_new_statistic) || (webui_mode == 'material' && !force_new_statistic) || (webui_mode == 'new' && force_old_statistic)) {
                $('#new_content').hide();
                $('#old_content').show();
            }
        }
