...
function sms_btnAddEvent() {
    $("#sms_table :checkbox").live("click", function() {
        if(webui_mode == 'old' && !force_new_sms || webui_mode == 'new' && force_old_sms || webui_mode == 'material') {
...
function sms_showWaitingDialog(tipTitle, tipContent, callback_func) {
    if(webui_mode == 'material') {
        var wait_div = "<h4 class='mdl-dialog__title'>" +
        tipTitle +
        "</h4>" +
        ...
        "<a href='javascript:void(0);' id='pop_Cancel' class='mdl-button'>"+common_cancel+"</a>"+
        "<a href='javascript:void(0);' id='pop_Close' class='mdl-button'>"+common_close+"</a>"+
        "</div>";
...
function sms_showSmsDialog() {
    if(webui_mode == 'material') {
        document.querySelectorAll('#sms_dialog').forEach(function(item, i, arr) {
            item.close();
        });
        $('#sms_dialog').remove();
        ...
        dialogHtml += "<h4 class='mdl-dialog__title'>" + sms_label_message + "</h4>";
        ...
        dialogHtml += "    <a href='javascript:void(0);' id='pop_send' class='mdl-button'>" + common_send + "</a>";
        dialogHtml += "    <a href='javascript:void(0);' id='pop_save_to_drafts' class='mdl-button'>" + sms_label_save_to_drafts + "</a>";
...
function creatSmsContent() {
    if(webui_mode == 'new' && force_old_sms) {
        ...
        if(webui_mode == 'old' && force_new_sms) {
            ...
            $('.content').prepend($('#new_smsmenu_list').detach().attr('style', 'margin-left: 23px;').show());
        } else {
...
function sms_getSmsData(smsCount, BoxType, Smstat, Index) {
    ...
    if (Index === undefined) {
        if (g_sms_smsList.length > 0) {
            setRightSmsText(g_sms_smsList[0]);
            setOpenSmsRead(g_sms_smsList[0].Index);
        } else {
            setRightSmsText(null);
        }
    } else {
