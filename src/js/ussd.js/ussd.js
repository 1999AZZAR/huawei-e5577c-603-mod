...
function creatUssdContent() {
    if(webui_mode == 'new' && force_old_ussd) {
        ...
        $('#new_ussd_command').remove();
        $('#new_ussd_receive').remove();
    } else {
        if(webui_mode == 'old' && force_new_ussd) {
            $('.content').empty();
            $('.content').append($('#new_smsmenu_list').detach().attr('style', 'margin-left: 28px;').show());
    ...
    if((webui_mode == 'new' && !force_old_ussd) || (webui_mode == 'old' && force_new_ussd) || g_ussdLeftmenu[0] == 'pregeneral') {
        clearAllErrorLabel();
        ussd_show_General(g_ussd_preconfig.General);
    }
}
...
function removeGeneralCommandDialog() {
	call_dialog(ussd_label_remove_command, '<p><b>' + ussd_label_click_to_remove + common_colon + '</b></p>' + removeGeneralCommandList() + '<br><div id="div_remove_default_ussd_cgi" style="display:none;"><input id="input_remove_default_ussd_cgi" type="checkbox"> ' + ussd_label_remove_default_commands_from_modem + '</div>', common_close, 'pop_Cancel');
...
function getGeneralCommandList(value) {
	...
	if(LANGUAGE_DATA.current_language == 'ru_ru' && value.RussianMenu) {
		if((webui_mode == 'old' && !force_new_ussd) || (webui_mode == 'new' && force_old_ussd) || webui_mode == 'material' || !value.RussianNewMenu) {
		...
		if((webui_mode == 'old' && !force_new_ussd) || (webui_mode == 'new' && force_old_ussd) || webui_mode == 'material' || !value.NewMenu) {
	...
	if (valueMenu) {
		if (valueMenu.MenuItem) {
			if ($.isArray(valueMenu.MenuItem)) {
				...
				if(general_cgi_command_list.rem_def_cmd_list.length == 0 && JSON.parse(localStorage.ussd_generalRemovedDefaultCommandList).length == 0) {
					config_list.push(formatGeneralCommandList(valueMenu.MenuItem, 'default', 0));
				}
			}
		}
	}
	...
	if (subValue.Name.length == 0) {
		...
		if(subValue.Command.length != 0) {
			if((webui_mode == 'new' && !force_old_ussd) || (webui_mode == 'old' && force_new_ussd)) {
				...
				if((webui_mode == 'old' && !force_new_ussd) || (webui_mode == 'new' && force_old_ussd) || webui_mode == 'material') {
