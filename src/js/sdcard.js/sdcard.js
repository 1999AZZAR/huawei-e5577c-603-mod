...
function folderlists(now_path) {
    ...
    saveAjaxData('api/sdcard/sdfile', path_xml, function($xml) {
        ...
        if ('response' == return_ret.type) {
                    ...
                    if (FILE_LIST_TYPE_FOLDER == file_array[i].Type) {
                        ...
                        if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
                            if (1024 <= file_size && (1024 * 1024) > file_size) {
                                file_size = common_unit_kb + ' ' + formatFloat(file_size / 1024, 2);
                            } else if ((1024 * 1024) <= file_size && (1024 * 1024 * 1024) > file_size) {
                                file_size = common_unit_mb + ' ' + formatFloat(file_size / (1024 * 1024), 2);
                            } else if ((1024 * 1024 * 1024) <= file_size && (1024 * 1024 * 1024 * 1024) > file_size) {
                                file_size = common_unit_gb + ' ' + formatFloat(file_size / (1024 * 1024 * 1024), 2);
                            } else if ((1024 * 1024 * 1024 * 1024) <= file_size) {
                                file_size = common_unit_tb + ' ' + formatFloat(file_size / (1024 * 1024 * 1024 * 1024), 2);
                            } else {
                                file_size = common_unit_byte + ' ' + formatFloat(file_size / 1, 2);
                            }
                        } else {
                            if (1024 <= file_size && (1024 * 1024) > file_size) {
                                file_size = formatFloat(file_size / 1024, 2) + ' ' + common_unit_kb;
                            } else if ((1024 * 1024) <= file_size && (1024 * 1024 * 1024) > file_size) {
                                file_size = formatFloat(file_size / (1024 * 1024), 2) + ' ' + common_unit_mb;
                            } else if ((1024 * 1024 * 1024) <= file_size && (1024 * 1024 * 1024 * 1024) > file_size) {
                                file_size = formatFloat(file_size / (1024 * 1024 * 1024), 2) + ' ' + common_unit_gb;
                            } else if ((1024 * 1024 * 1024 * 1024) <= file_size) {
                                file_size = formatFloat(file_size / (1024 * 1024 * 1024 * 1024), 2) + ' ' + common_unit_tb;
                            } else {
                                file_size = formatFloat(file_size / 1, 2) + ' ' + common_unit_byte;
                            }
                        }
                        if (now_path === '/') {
            ...
            if ('undefined' != typeof(return_ret.error) && MACRO_FILE_NAME_LONGERR == return_ret.error.code) {
                button_enable("button_sd_upload", "1");
                button_enable("sd_new_folder", "1");
                setTimeout( function() {
                    showInfoDialog(IDS_sd_file_name_longer);
                }, 600);
            } else if ('undefined' != typeof(return_ret.error) && MACRO_NO_ACCESS_RIGHTS == return_ret.error.code) {
                setTimeout( function() {
    ...
    getAjaxData('api/sdcard/sdcapacity', function($xml) {
        ...
        if ('response' == cap_ret.type) {
            ...
            if (1024 < available2MB) {
                ...
                if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
                    $('#availablesize').html(common_unit_gb + ' ' + available2MB);
                } else {
                    $('#availablesize').html(available2MB + ' ' + common_unit_gb);
                }
            } else {
                if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
                    $('#availablesize').html( common_unit_mb + ' ' + formatFloat(available2MB, 2));
                } else {
                    $('#availablesize').html(formatFloat(available2MB, 2) + ' ' + common_unit_mb);
                }
            }
            ...
            if (1024 < totalsize2MB) {
                ...
                if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
                    $('#totalsize').html(common_unit_gb + ' ' + totalsize2MB);
                } else {
                    $('#totalsize').html(totalsize2MB + ' ' + common_unit_gb);
                }
            } else {
                if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa') {
                    $('#totalsize').html( common_unit_mb + ' ' + formatFloat(totalsize2MB, 2));
                } else {
                    $('#totalsize').html(formatFloat(totalsize2MB, 2) + ' ' + common_unit_mb);
                }
            }
...
$(document).ready( function() {
    ...
    $('#sd_list :checkbox').live('click', function() {
        ...
        if(this == $allCheckBox[0]) {
            ...
            $allCheckBox.eq(0).attr('checked', allChecked);
        }
        checkedCount = $allCheckBox.filter(":not(:first)").filter(":checked").length;
...
function showConfirmDialog_sdcard(content, okFunc, cancelFunc, removeable,removeFunc) {
    ...
    if(webui_mode == 'material') {
        ...
        dialogHtml += '</div>';
        dialogHtml += "<div class='mdl-dialog__actions'>";
        dialogHtml += "    <a href='javascript:void(0);' id='pop_confirm' class='mdl-button'>"+common_yes+"</a>";
        if (cancelFunc != '' && cancelFunc != ' ' && cancelFunc != null) {
            dialogHtml += "    <a href='javascript:void(0);' id='pop_Cancel' class='mdl-button'>"+common_no+"</a>";
        }
        dialogHtml += '</div>';
    } else {
        if (DIALOG_UNREMOVE != removeable) {
