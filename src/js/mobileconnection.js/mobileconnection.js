...
function setRoamStatus(_enable) {
    ...
    $("input[name='roam_switch']").eq(_enable).attr('checked', true);
}
...
function setConnectionStatus() {
    if (CONNECTMODE_MANUAL == $('#select_connection_mode').val() && null == g_mobile_dataswitch) {
        ...
        if(g_connectionData.pdp_always_on == 1) {
            $("#select_max_idle_time option[value='0']").remove();
...
function initPage() {
    ...
    getAjaxData('api/dialup/connection', function($xml) {
        ...
        if (ret.type == 'response') {
            ...
            $('#enable_auto_dial_switch').attr('checked', (g_connectionData.auto_dial_switch == 1) ? true : false);
        }
